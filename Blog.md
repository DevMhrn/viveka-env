...
Most agent benchmarks score "did the agent finish the task." That signal cannot distinguish a model that completes a task safely from one that completes it by doing the one thing it was never supposed to do. Viveka's reward has six components, each designed to close a specific gameable hole.
